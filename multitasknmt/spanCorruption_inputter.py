...
class SpanCorruptionInputter(ExampleInputter):
    ...
    def _make_dataset(self, data_file, training=None):
        ...
        srcDatasets = self.inputters[0].make_dataset(data_file[0], training=training)
        if not isinstance(srcDatasets, list): srcDatasets = [srcDatasets]
        parallel_datasets = []
        for d in srcDatasets:
            parallel_datasets.append(self.spanCorruptionGenerator.t5_denoise(d,
    ...
    def get_dataset_size(self, data_file):
        ...
        size = self.inputters[0].get_dataset_size(data_file[0])
        if size is not None:
