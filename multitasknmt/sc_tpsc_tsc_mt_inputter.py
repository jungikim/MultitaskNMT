...
class ScTpscTscMtInputter(SpanCorruptionInputter):
    def __init__(
        self,
        features_inputter,
        labels_inputter,
    ...
    def _make_dataset(self, data_file, training=None):
        ...
        if bilingual_data_file and len(bilingual_data_file) == 2 and len(bilingual_data_file[0]) > 0:
            ...
            # mt_datasets = []
            # mt_num_files = -1
            # for i, (inputter, data) in enumerate(zip(self.inputters, bilingual_data_file)):
            #     mt_dataset = inputter.make_dataset(data, training=training)
            #     if not isinstance(mt_dataset, list):
            #         mt_dataset = [mt_dataset]
            #     mt_datasets.append(mt_dataset)
            #     if mt_num_files < 0:
            #         mt_num_files = len(mt_dataset)
            #     elif len(mt_dataset) != mt_num_files:
            #         raise ValueError(
            #             "All parallel inputs must have the same number of data files, "
            #             "saw %d files for input 0 but got %d files for input %d"
            #             % (mt_num_files, len(mt_dataset), i)
            #         )
            # mt_parallel_datasets = [
